...
void VideoEnc::acknowledge(unsigned int ackSeqNr, float currentTime) {
    ...
    float rttSample = currentTime - pkt.sendTime;
...
int VideoEnc::processAck(unsigned int ackedSeqNr, float currentTime) {
    ...
    for (auto it = unacked_.begin(); it != ackedIt; ++it) {
        ...
        float timeSinceLastSend = currentTime - pkt.lastSendTime;
        if (pkt.numRetx == 0 || timeSinceLastSend > ewmaRttSec) {
            pkt.numRetx++;
            pkt.lastSendTime = currentTime;
            numToRetransmit++;
...
int VideoEnc::encode(float time) {
    ...
    if (!unacked_.empty()) {
        ...
        float timeSinceFirstSend = time - firstUnacked.sendTime;
    ...
    while (tmp2 > 0) {
        ...
        unackedPkt.sendTime = time;
        unackedPkt.lastSendTime = time;
