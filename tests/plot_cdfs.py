...
def main() -> None:
    ...
    sender_plots = [
        ...
        ("ecn_ce_pct", "CE Marking [%]", "Sender CE Marking CDF"),
    ]

    receiver_plots = [
        (
            "receive_rate_avg_mbps",
            "Average Receive Rate [Mbps]",
            "Receiver Average Receive Rate CDF",
        ),
        ...
        (
            "freeze_duration_total_s",
            "Total Freeze Duration [s]",
            "Receiver Freeze Duration CDF",
        ),
        ("freeze_count_total", "Freeze Count", "Receiver Freeze Count CDF"),
        (
            "inter_frame_delay_variance_ms2",
            "Inter-frame Delay Variance [ms^2]",
            "Receiver IF Delay Variance CDF",
        ),
    ]

    made = 0

    for key, xlabel, title in sender_plots:
        ...
        make_cdf(
            ...
            xlabel,
            title,
            out / "sender" / f"sender_cdf_{key}.png",
        )
        made += 1

    for key, xlabel, title in receiver_plots:
        ...
        make_cdf(
            ...
            xlabel,
            title,
            out / "receiver" / f"receiver_cdf_{key}.png",
        )
        made += 1

    print(f"Generated {made} CDF plots in {out}")
