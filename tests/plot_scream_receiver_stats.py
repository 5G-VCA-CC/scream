...
def parse_stats_file(filename):
    ...
    for block in blocks:
        ...
        for key, pattern in patterns.items():
            ...
            if match:
                stats[key][-1] = float(match.group(1))
...
def parse_stats_path(input_path: str):
    ...
    for f in files:
        ...
        run_files.append(f)
        run_breaks.append(len(combined["datagrams_received"]))
...
def plot_histograms(stats, output_prefix="stats"):
    ...
    plot_single_hist(stats["receive_rate_mbps"], "Distribution of Receive Rate", "Receive Rate (Mbps)", "green", f"{output_prefix}_hist_receive_rate.png")
    plot_single_hist(stats["freeze_duration"], "Distribution of Freeze Duration per Interval", "Freeze Duration (s)", "red", f"{output_prefix}_hist_freeze_duration.png")
    plot_single_hist(stats["inter_frame_delay_difference"], "Distribution of Inter-Frame Delay difference", "difference (s)", "purple", f"{output_prefix}_hist_difference.png")
